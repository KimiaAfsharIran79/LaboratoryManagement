        ...
        private void grdTests_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 8)
            {
                if (MessageBox.Show("آیا از حذف مطمئنید؟", "هشدار", MessageBoxButtons.OKCancel) == DialogResult.OK)
                {
                    ...
                    {
                        MessageBox.Show("این آزمایش در جزئیات سربرگ یک بیمار ثبت شده است.");
                    }
                }
            }
            if (e.ColumnIndex == 7)
            {
                ...
                var t = repo.Get(id);
                GotoEditMode();
                if (t.AgeHasEffect) chkAge.Checked = true; else chkAge.Checked = false;
                if (t.GenderHasEffect) chkGender.Checked = true; else chkGender.Checked = false;
